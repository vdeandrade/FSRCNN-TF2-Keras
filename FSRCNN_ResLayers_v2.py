#!/usr/bin/env python
"""
    This version used shared_axes with PReLU activation layer to share the learned parameters.
    ...
    The version 2 adds a residual connection after the shrinking layer, combining its output
    with the appropriately aligned output of the feature extraction layer.
    The goal is to try to retain more textural information
...
def residual_block(input_tensor, filters):
    ...
    if input_tensor.shape[-1] != filters:  # If channels mismatch
...
# Residual block after shrinking layer
model = residual_block(model, filters=56)  # Match filters to feature_extraction

# Add residual blocks in the mapping layers
for _ in range(4):  # Add 4 residual blocks
    model = residual_block(model, filters=16)
